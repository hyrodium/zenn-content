...
# はじめに
四元数(quaternion)とは$q = w+ix+jy+kz$のように表される数のことで、複素数を拡張したものと考えることができます。
...
2022年2月くらいから[Sethさん](https://github.com/sethaxen)と私がメンテナ権限を貰ってメンテナンスを続けてきました。
最近[v0.7.0をリリース](https://github.com/JuliaGeometry/Quaternions.jl/releases/tag/v0.7.0)して、Quaternions.jlはようやく使える形に整ってきた感じがあります。
本記事ではQuaternions.jlのメンテナンスでの変更点や教訓などを記述しようと思います。

# メンテナンスでの変更点
私が開発に参加したのはv0.4.6辺りからです。最新リリースがv0.7.0なので、破壊的変更を伴ったリリースを3回行ったことになります。[^1]
以降ではこれらの変更の概要を説明します。
...
のように実装されていましたが、現在の実装ではこの`norm`のフィールド[^3]が削除されています。
...
julia> quat(1,2,0,0) + Complex(1,2)
Quaternion{Int64}(2, 4, 0, 0, false)

julia> convert(Complex, quat(1,2,0,0))
...
Closest candidates are:
  ...
  ...
Stacktrace:
 [1] convert(#unused#::Type{Complex}, x::Quaternion{Int64})
   @ Base ./number.jl:7
 [2] top-level scope
   @ REPL[4]:1
```
...
* $\mathbb{C}\to\mathbb{H}$として自然なものを1つ選ぶことはできない
...
* `LinearAlgebra.normalize(::Quaternion)`の削除
  * `normalize(::Number)`実装されておらず、`sign(::Number)`を使うのが一般的
...
* `rotationmatrix`などの削除
  * 従来は四元数から回転行列に変換($U(1,\mathbb{H}) \to SO(3)$)するための関数が用意されていた
  * 戻り値が`Matrix{Float64}`であって`StaticMatrix`でないので非効率。
