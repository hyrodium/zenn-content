...
# はじめに
四元数(quaternion)とは$q = w+ix+jy+kz$のように表される数のことで、複素数を拡張したものです。
...
# メンテナンスでの変更点
私が開発に参加したのはv0.4.2辺りからです。最新リリースがv0.7.0なので、3回の破壊的変更を伴ったリリースを3回行ったことになります。[^1]
以降ではこれらの変更の概要を説明します。
...
[^1]: Semantic versioningです。v0.Y.ZではYのincrementで破壊的変更、Zのincrementで機能追加・バグ修正を表します。
...
のように実装されていましたが、現在の実装ではこの`norm`のフィールドが削除されています。
...
[^3]: ノルム(実数)というよりは`normalized`の略と考えた方が良いです。`true`で単位四元数を表します。
