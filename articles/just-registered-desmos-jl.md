...
using GLM
nt = (;x1,y1)
fit(LinearModel, @formula(y1 ~ x1 + x1*x1), nt)
```

実行後には以下のような表が示され、確かに前述の係数が`Coef.`の列に記載されていることが分かりますね。[^9]

[^9]: 個人的な感想ですが、`@formula(y1 ~ x1 + x1*x1)`の記法では、どこに係数が挿入されて線形回帰されるのか直感的でないと感じてしまいました。Desmosの記法は未定義変数がそのまま回帰で決定される係数になるので分かりやすいですが、GLM.jlでそのまま採用すべきと思うわけでもないので悩ましいです。[GLM.jlのドキュメント](https://juliastats.org/GLM.jl/v1.9.0/examples/)ではR言語との比較が記載されているので、私がR言語に慣れていないだけの可能性はあります。
...
x1 & x1      -1.29853     0.0400957  -32.39    <1e-53  -1.37811    -1.21895
────────────────────────────────────────────────────────────────────────────
```

Desmos.jlを使う場合のメリットは以下のようになるでしょう。
...
GLM.jlは統計分析目的、Desmos.jlは可視化・教育目的として、必要に応じて使い分けるのが便利でしょう。
...
- Desmos.jlをPlots.jlのバックエンドとして使いたいね
	- Plots.jlのメンテナ不足問題もあるらしく、そこのお手伝いもやってみたい
