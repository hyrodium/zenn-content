...
この一連の流れにおいて **3の「切り出し形状の計算」** が一番非自明で、この形状を計算するパッケージが[ElasticSurfaceEmbedding.jl](https://github.com/hyrodium/ElasticSurfaceEmbedding.jl)です。あとでもう少し詳しく説明しますが、この形状は「歪エネルギー[^1]を最小にする2次元Euclid空間への埋め込み」として特徴づけられています。"ElasticSurfaceEmbedding"の名前にはこの意味が込められています。

[^1]: 通常はひずみエネルギーと書かれることが多いですが、筆者の好みで平仮名を避けています。
...
この形状[^2]をレーザー加工機で切り出し、編み上げれば放物面が完成します！
...
ElasticSurfaceEmbedding.𝒑₍₀₎(u¹,u²) = SVector(u¹, u², u¹^2+u²^2)
```
は曲面の形状を定義です。少し気持ち悪いですが、メソッドの上書きによって形状を定義しています。[^3]
[^3]: 引数に関数を入れるように実装したことがありましたが、実行速度の低下があったので断念しました。詳細な速度低下の原因は未調査で、今後時間があれば修正される可能性があります。ただ、複数の形状を同時に計算しないはずなので、メソッドの上書きでも大きな問題にはなりません。
...
## 歪エネルギー最小化
平面材料を変形させて曲面片をつくるとき、材料内に引張と圧縮が発生します。この変形に係るエネルギーが歪エネルギーであり、これを最小化するような切り出し形状が最適な形状だと定義します。

![](https://storage.googleapis.com/zenn-user-upload/4e65b1570872-20221129.png)

ただし、材料の変形としては「平面形状を曲面片に変形」よりも「曲面片を平面形状に変形」の方が直観的に扱いやすいです。

![](https://storage.googleapis.com/zenn-user-upload/99ae1f10c638-20221129.png)

引張と圧縮の位置こそ入れ替わりますが、歪エネルギーはほぼ同じ[^4]なので変形の向きを入れ替えても問題ありません。

[^4]: ほぼというのは「材料非線形性を無視する限りは」という意味です。

## B-spline多様体
これまでに、YouTube・Juliaパッケージのドキュメント・PDF資料にB-splineの解説を書いてきました。詳しくは以下を参照してください！

* [B-spline入門（線形代数がすこし分かる人向け）](https://www.youtube.com/watch?v=GOdY02PA_WI&t=111s)
...
![](https://storage.googleapis.com/zenn-user-upload/71cd2a504742-20221129.png)

歪エネルギー最小化の問題は、弱形式の偏微分方程式として与えられます。Galerkin法で解いているので通常のFEMのように3角形メッシュで解いても良かったのですが、本研究ではB-splineを使っています。以下がその理由です：
...
```
export_all_steps(".",allsteps, xlims=(-2,2), ylims=(-1,1))
```

例えば`i=9`の曲面片に対しては以下のように、歪が徐々に緩和されていく様子が見て取れます。
